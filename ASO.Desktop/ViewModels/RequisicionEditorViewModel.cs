...
public sealed class RequisicionEditorViewModel : CrudEditorViewModelBase<Requisicion>
{
    ...
    public RequisicionEditorViewModel(Requisicion original,
    ...
    private string _tipoLubricanteTexto = string.Empty;
    public string TipoLubricanteTexto
    {
        get => _tipoLubricanteTexto;
        set => SetProperty(ref _tipoLubricanteTexto, value);
    }
    ...
    /// <summary>Unidad de flota a la que se destina el repuesto. Opcional: se puede dejar sin
    /// elegir cuando el repuesto es para reponer almacén, no una máquina en concreto.</summary>
    private ActivoFlota? _activoLineaSeleccionado;
    public ActivoFlota? ActivoLineaSeleccionado
    {
        get => _activoLineaSeleccionado;
        set => SetProperty(ref _activoLineaSeleccionado, value);
    }
    ...
    private void AgregarLinea()
    {
        ...
        if (TipoLineaSeleccionado == TipoInsumo.Combustible)
        {
            string? grado = null;
            if (TipoCombustibleLineaSeleccionado == TipoCombustible.Lubricante)
            {
                if (string.IsNullOrWhiteSpace(TipoLubricanteTexto))
                {
                    ErrorValidacion = "Indique el grado del lubricante (por ejemplo, 20W50).";
                    return;
                }

                grado = TipoLubricanteTexto.Trim();
            }

            Lineas.Add(new RequisicionLinea
            {
                TipoInsumo = TipoInsumo.Combustible,
                TipoCombustibleSolicitado = TipoCombustibleLineaSeleccionado,
                TipoLubricante = grado,
                Cantidad = cantidad,
                UnidadTexto = "L"
            });
        }
        else
        {
            ...
            Lineas.Add(new RequisicionLinea
            {
                ...
                ActivoId = ActivoLineaSeleccionado?.Id,
                ActivoEtiqueta = ActivoLineaSeleccionado?.Etiqueta ?? string.Empty,
                Cantidad = cantidad,
        ...
        TipoLubricanteTexto = string.Empty;
        ActivoLineaSeleccionado = null;
    }
